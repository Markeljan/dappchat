...
export function getChainByName(name: string) {
  ...
  const { bestMatch: { target } } = stringSimilarity.findBestMatch(lowerCaseName, chainsArray.map(chain => chain.name.toLowerCase()));
  const closestChain = chainsArray.find(chain => chain.name.toLowerCase() === target);
